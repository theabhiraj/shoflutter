...
class _ExpensesReportsState extends State<ExpensesReports> {
  ...
  Map<String, dynamic> _expensesData = {};
  ...
  void initState() {
    ...
    _monthController.text = DateFormat('MMMM').format(_selectedDate);
    if (widget.initialData != null) {
      setState(() {
        _expensesData = widget.initialData!;
        _isLoading = false;
      });
    } else {
      _loadExpensesData();
  ...
  Future<void> _loadExpensesData() async {
    try {
      final ref = FirebaseDatabase.instance
          .ref('shop_management/shop_1/reports/expenses')
      ...
      if (snapshot.exists) {
        setState(() {
          _expensesData = Map<String, dynamic>.from(snapshot.value as Map);
          _isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> _selectMonth() async {
    ...
    if (picked != null && picked != _selectedDate) {
      setState(() {
        ...
        _monthController.text = DateFormat('MMMM').format(picked);
      });
      _loadExpensesData();
    }
  }
  ...
  Widget _buildExpensesOverview() {
    final totalExpenses = _expensesData['total_expenses']?.toString() ?? '0';
    final topExpenseCategory =
        _expensesData['top_expense_category']?.toString() ?? 'N/A';
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Expenses Overview - ${DateFormat('MMMM yyyy').format(_selectedDate)}',
          ...
          children: [
            ...
            Wrap(
              ...
              children: categories.keys
                  .map((category) => _buildChartLegend(
                        category,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CommonAppBar(title: 'Expenses Reports'),
      body: Container(
        decoration: BoxDecoration(
